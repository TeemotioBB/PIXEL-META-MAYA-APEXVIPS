...
def vincular_tracking_por_uid_temp(uid_real: int, uid_temp: str) -> bool:
    ...
    try:
        ...
        if "fbp" in tracking_data: r.set(f"fbp:{uid_real}", tracking_data["fbp"], ex=604800)
        if "fbc" in tracking_data: r.set(f"fbc:{uid_real}", tracking_data["fbc"], ex=604800)
        if "client_ip" in tracking_data: r.set(f"ip:{uid_real}", tracking_data["client_ip"], ex=604800)
        if "client_user_agent" in tracking_data: r.set(f"ua:{uid_real}", tracking_data["client_user_agent"], ex=604800)
...
def apex_joined_fallback(uid: int):
    # Aguarda um tempo para dar margem entre o clique na página e a entrada no bot
    time.sleep(20)
    ...
    try:
        ...
        for key in keys:
            ...
            # Cruza os dados do Apex com os do Redis
...
def apex_tracking():
    ...
    tracking_data = {}
    if fbp: tracking_data["fbp"] = fbp
    if fbc: tracking_data["fbc"] = fbc
    elif fbclid: tracking_data["fbc"] = f"fb.1.{int(time.time() * 1000)}.{fbclid}"
    if client_ip: tracking_data["client_ip"] = client_ip.split(',')[0].strip()
    if client_user_agent: tracking_data["client_user_agent"] = client_user_agent
...
def apex_webhook():
    ...
    if evento == "user_joined":
        ...
        logger.info(f"🏁 [APEX JOINED] pending_join:{uid} criada — iniciando cruzamento fallback")
        # Inicia a caçada aos dados de pixel de quem clicou na página
...
def home():
    return "🚀 Servidor CAPI Ativo (Gerenciamento do Bot transferido para o Apex)", 200
